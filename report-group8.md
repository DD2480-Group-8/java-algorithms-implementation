...
**Q:** Is the high complexity you identified really necessary? Is it possible to split up the code (in the five complex functions you have identified) into smaller units to reduce complexity? If so, how would you go about this? Document your plan.
- The high complexity is due to the fact that there are many different cases that needs to be tested. Children to the parent node needs to be checked if they are all lesser or all greater, as well as check if they are not invalid in some sort.

- There are multiple if-statements in the function BinaryHeapArray:heapDown. The function could be refactored to move all of the bigger if statements into help-functions. This would lead to heapDown reducing the cyclomatic complexity by a lot, since the if-statements are the things that drives it tp the high value as it is now. But this will lead to that the help-function will have heigher than the original function. The if-statement in that would be moved would be the one starting with:
   ```Java
   ...
   ```
   As the array is a class variable, only the leftIndex, left, rightIndex and right need to be sent as parameters.
...
The following command can be executed to shows the difference in the PatriciaTrie::addSequence function after the instrumentation as been added:
```
git diff 3c4783a8299acdc7d583f63a4c70c8678cf5c6ef e97279282f4b3567b9f92bf4ac372772d34244da
```
...
Test cases added:  
```Java
testPatriciaTrieAddExistingBlack()  
testPatriciaTrieAddDuplicates()  
```
...
Test cases added:
```
testMinHeapRemoveWithDuplicates()
testMaxHeapRemoveWithDuplicates()
division()
testArrayListSetAndToString()
```
...
diff:
```
34255fb5be88aaa486a12f42df3d75d251460daf
```
